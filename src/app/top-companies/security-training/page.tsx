...
export default function TopSecurityTrainingPage() {
  const proCategory = 'security-training';
  const pros = allProfessionals[proCategory] || [];
  const topCompanies = pros.sort((a, b) => b.rating - a.rating).slice(0, 5);

  const heroImage = CategoryImages.find(p => p.id === 'security-training-image');
         ...
         <section className="relative min-h-[500px] flex items-center justify-center text-center text-white">
            {heroImage && (
                <Image
                    ...
                    alt={heroImage.description || "Security Training service background"}
                    fill
             ...
             <div className="relative container mx-auto px-4 grid md:grid-cols-2 items-center gap-8 text-left">
                <div className="hidden md:block">
                  <h1 className="text-4xl md:text-5xl font-normal">Top Security Training Centres</h1>
                  <p className="mt-4 text-lg text-white/90">
                      PSIRA accredited security training for all grades.
                  </p>
                </div>
          ...
          <div className="container mx-auto px-4">
            <div className="text-left mb-8">
                <div className="flex items-center gap-1 text-sm text-muted-foreground">
                    ...
                    <span className="font-medium text-foreground">Top Security Training Centres</span>
                </div>
                <h2 className="text-3xl mt-1">Top Security Training Centres in Gauteng</h2>
            </div>
            <div className="grid lg:grid-cols-3 gap-12">
                ...
                <aside className="space-y-8">
                    <div className="p-6 border rounded-lg bg-card">
                        <h3 className="mb-3 font-semibold text-foreground">Need Security Training?</h3>
                        <ul className="list-disc list-inside text-sm text-muted-foreground space-y-1">
                            <li>{pros.reduce((acc, pro) => acc + (pro.reviews || 0), 0)}+ Reviews for security training</li>
                            <li>{pros.filter(p => p.rating >= 4).length * 10}+ Positive Reviews</li>
                            <li>Recently hired centres have been rated 4.9/5 stars by students</li>
                            <li>View Top Security Training Centres today</li>
                        </ul>
                    </div>
                     <div className="p-6 border rounded-lg bg-card">
