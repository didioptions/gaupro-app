...
export default function TopInsurancePage() {
  const proCategory = 'insurance';
  const pros = allProfessionals[proCategory] || [];
  const topCompanies = pros.sort((a, b) => b.rating - a.rating).slice(0, 5);

  const heroImage = CategoryImages.find(p => p.id === 'insurance-image');
         ...
         <section className="relative min-h-[500px] flex items-center justify-center text-center text-white">
            {heroImage && (
                <Image
                    ...
                    alt={heroImage.description || "Insurance service background"}
                    fill
             ...
             <div className="relative container mx-auto px-4 grid md:grid-cols-2 items-center gap-8 text-left">
                <div className="hidden md:block">
                  <h1 className="text-4xl md:text-5xl font-normal">Top Insurance Brokers</h1>
                  <p className="mt-4 text-lg text-white/90">
                      Compare quotes and find the best cover for your car, home, or business.
                  </p>
                </div>
          ...
          <div className="container mx-auto px-4">
            <div className="text-left mb-8">
                <div className="flex items-center gap-1 text-sm text-muted-foreground">
                    ...
                    <span className="font-medium text-foreground">Top Insurance Brokers</span>
                </div>
                <h2 className="text-3xl mt-1">Top Insurance Brokers in Gauteng</h2>
            </div>
            <div className="grid lg:grid-cols-3 gap-12">
                ...
                <aside className="space-y-8">
                    <div className="p-6 border rounded-lg bg-card">
                        <h3 className="mb-3 font-semibold text-foreground">Need Insurance?</h3>
                        <ul className="list-disc list-inside text-sm text-muted-foreground space-y-1">
                            <li>{pros.reduce((acc, pro) => acc + (pro.reviews || 0), 0)}+ Reviews for insurance brokers</li>
                            ...
                            <li>View Top Insurance Brokers today</li>
                        </ul>
                    </div>
                     <div className="p-6 border rounded-lg bg-card">
