...
export default function TopPrecastFencingPage() {
  const proCategory = 'precast-fencing';
  const pros = allProfessionals[proCategory] || [];
  const topCompanies = pros.sort((a, b) => b.rating - a.rating).slice(0, 6);

  const heroImage = CategoryImages.find(p => p.id === 'precast-fencing-image');
         ...
         <section className="relative min-h-[500px] flex items-center justify-center text-center text-white">
            {heroImage && (
                <Image
                    ...
                    alt={heroImage.description || "Precast fencing service background"}
                    fill
             ...
             <div className="relative container mx-auto px-4 grid md:grid-cols-2 items-center gap-8 text-left">
                <div className="hidden md:block">
                  <h1 className="text-4xl md:text-5xl font-normal">Top Precast Fencing Companies</h1>
                  <p className="mt-4 text-lg text-white/90">
                      Durable and cost-effective precast concrete walling solutions.
                  </p>
                </div>
          ...
          <div className="container mx-auto px-4">
            <div className="text-left mb-8">
                <div className="flex items-center gap-1 text-sm text-muted-foreground">
                    ...
                    <span className="font-medium text-foreground">Top Precast Fencing Companies</span>
                </div>
                <h2 className="text-3xl mt-1">Top 6 Precast Fencing Companies</h2>
            </div>
            <div className="grid lg:grid-cols-3 gap-12">
                ...
                <aside className="space-y-8">
                    <div className="p-6 border rounded-lg bg-card">
                        <h3 className="mb-3 font-semibold text-foreground">Need Precast Fencing?</h3>
                        <ul className="list-disc list-inside text-sm text-muted-foreground space-y-1">
                            <li>{pros.reduce((acc, pro) => acc + pro.reviews, 0)}+ Reviews for precast fencing</li>
                            ...
                            <li>View Top Precast Fencing Companies today</li>
                        </ul>
                    </div>
                     <div className="p-6 border rounded-lg bg-card">
